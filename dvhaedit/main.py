...
from dvhaedit.threads import (
    ParsingProgressFrame,
    RefSyncProgressFrame,
    ValueGenProgressFrame,
...
class MainFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.input["preview"] = wx.TextCtrl(
            self, wx.ID_ANY, "", style=wx.TE_READONLY
        )
        self.input["selected_file"] = wx.ComboBox(
            self, wx.ID_ANY, style=wx.CB_DROPDOWN | wx.CB_READONLY
        )
        self.input_text_obj = [
            self.input[key] for key in keys
        ]  # use for text event binding and focusing
        ...
        self.button = {
            key: wx.Button(self, wx.ID_ANY, key.replace("_", " ").title())
            for key in keys
        }
        ...
        self.button["value_help"] = wx.BitmapButton(
        ...
        self.label = {
            key: wx.StaticText(
                self, wx.ID_ANY, key.replace("_", " ").title() + ":"
            )
            for key in keys
        }
        ...
        self.retain_rel_dir = wx.CheckBox(
    ...
    def __set_properties(self):
        ...
        self.button["search"].SetToolTip(
            "Search for DICOM tag based on keyword."
        )
        self.input["preview"].SetToolTip(
            "Values may be set dynamically, a preview is shown here. Note "
            "that generated UIDs may be different than the final value if no "
        ...
        msg = (
            'The "Value" in the tag editor below will be prepopulated with '
            "the value found for the specified tag in this file."
        )
    ...
    def __do_bind(self):
        ...
        # This requires that there is a function on_<button-key> for every
        # button, simplifies bind
        ...
        self.Bind(
            wx.EVT_COMBOBOX,
            self.on_file_select,
            id=self.input["selected_file"].GetId(),
        )
    ...
    def __do_layout(self):
        ...
        sizer_input_dir_wrapper = wx.StaticBoxSizer(
            wx.StaticBox(self, wx.ID_ANY, "Directory"), wx.VERTICAL
        )
        sizer_input_dir = wx.BoxSizer(wx.HORIZONTAL)
        sizer_input_file = wx.BoxSizer(wx.VERTICAL)
        sizer_edit_wrapper = wx.StaticBoxSizer(
            wx.StaticBox(self, wx.ID_ANY, "Tag Editor"), wx.VERTICAL
        )
        ...
        sizer_output_dir_wrapper = wx.StaticBoxSizer(
            wx.StaticBox(self, wx.ID_ANY, "Output Directory"), wx.HORIZONTAL
        )
        ...
        sizer_input_file.Add(
            row_sizer, 1, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, 5
        )
        sizer_input_file.Add(
            self.label["selected_file"], 0, wx.LEFT | wx.TOP, 5
        )
        sizer_input_file.Add(
            self.input["selected_file"], 0, wx.EXPAND | wx.LEFT | wx.BOTTOM, 5
        )
        ...
        for key in [
            ...
            obj = (
                self.button
                if key in {"search", "value_help", "advanced"}
                else self.input
            )
            ...
            sizer_edit.Add(
                sizer_edit_widgets[key], proportion, wx.EXPAND | wx.ALL, 5
            )
        ...
        row_sizer_value_help.Add(
            self.input["value"], 1, wx.EXPAND | wx.LEFT | wx.RIGHT, 5
        )
        ...
        sizer_edit_wrapper.Add(
            sizer_edit_buttons, 0, wx.ALIGN_CENTER | wx.ALL, 5
        )
        ...
        sizer_output_dir_inner_wrapper.Add(
            sizer_output_dir, 0, wx.ALL | wx.EXPAND, 5
        )
        sizer_output_dir_prepend.Add(
            self.label["prepend_file_name"], 0, wx.TOP | wx.LEFT | wx.RIGHT, 5
        )
        sizer_output_dir_prepend.Add(
            self.input["prepend_file_name"], 1, wx.EXPAND | wx.RIGHT, 5
        )
        sizer_output_dir_inner_wrapper.Add(
            sizer_output_dir_prepend,
            0,
            wx.EXPAND | wx.LEFT | wx.BOTTOM | wx.RIGHT,
            5,
        )
        sizer_output_dir_wrapper.Add(
            sizer_output_dir_inner_wrapper, 1, wx.EXPAND, 5
        )
        sizer_output_dir_wrapper.Add(
            self.button["out_browse"], 0, wx.ALIGN_TOP | wx.TOP, 10
        )
        sizer_main.Add(sizer_output_dir_wrapper, 0, wx.EXPAND | wx.ALL, 5)

        sizer_app_buttons.Add(
            self.update_referenced_tags, 0, wx.ALIGN_LEFT | wx.ALL, 5
        )
        sizer_app_buttons.Add(
            (20, 20), 1, wx.EXPAND | wx.ALIGN_LEFT | wx.ALL, 5
        )
        sizer_app_buttons.Add(
            self.button["save_dicom"], 0, wx.ALIGN_RIGHT | wx.ALL, 5
        )
        sizer_app_buttons.Add(
            self.button["quit"], 0, wx.ALIGN_RIGHT | wx.ALL, 5
        )
    ...
    # Binding occurs abstractly with a for loop, all buttons automatically
    # bind to a function of its dict key prepended with on_
    # For example, the Add button is self.button['add'], clicking it will
    # activate self.on_add
    ###########################################################################
    ...
    def on_browse(self, obj):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            # Reset if background was orange, DirDialog forces a valid
            # directory
            obj.ChangeValue(
                new_dir
            )  # Update TextCtrl without signaling a change event
    ...
    def on_save_template(self, *evt):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            save_data = {
                "table": self.data_table.get_save_data(),
    ...
    def on_load_template(self, *evt):
        ...
        dlg = wx.FileDialog(
            self, "Load template", "", wildcard="*.pickle", style=wx.FD_OPEN
        )
        if dlg.ShowModal() == wx.ID_OK:
            try:
    ...
    def on_save_dicom(self, *evt):
        ...
        if self.dir_contents_have_changed:
            with AskYesNo(
                self, "Directory contents have changed. Continue anyway?"
            ) as dlg:
    ...
    def on_selection(self, *evt):
        ...
        if selected_data:
            ...
            self.update_preview()
            self.current_options = deepcopy(
                self.all_options[selected_data[0][0]]
            )
    ...
    def on_key_up(self, evt):
        ...
        if keycode == wx.WXK_TAB:
            self.on_tab_key(evt)
        elif evt.GetEventObject() in {
            self.input["tag_group"],
    ...
    def on_tab_key(self, evt):
        ...
        if obj in {self.input["in_dir"], self.input["out_dir"]}:
            ...
            if isdir(new_dir):
                ...
                ErrorDialog(
                    self, "Please enter a valid directory.", "Directory Error"
                )
    ...
    def on_key_down_dir(self, evt):
        """Called on any key down in dir TextCtrl, only act on Enter/Return"""
        keycode = evt.GetKeyCode()
        obj = evt.GetEventObject()
    ...
    def update_dir_obj_text_color(self, evt):
        """Set dir TextCtrl background to orange if directory is invalid"""
        obj = evt.GetEventObject()
    ...
    def update_save_dicom_enable(self):
        """Disable save dicom button if there is not enough info provided"""
        enable = (
            isdir(self.input["in_dir"].GetValue())
            and isdir(self.input["out_dir"].GetValue())
            and self.data_table_has_data
        )
    ...
    def update_add_enable(self, *evt):
        enable = (
            ...
            and bool(self.group)
            and bool(self.element)
            and self.value_is_valid
        )
        self.button["add"].Enable(enable)
        value_str = self.input["value"].GetValue()
        adv_enable = enable and any(
            [v in value_str for v in ["*fuid[", "*vuid", "*frand[", "*vrand"]]
        )
    ...
    def update_combobox_files(self):
        """Update combobox with the file names found in the current in dir"""
    ...
    def update_init_value(self):
        """Update Value in the Tag Editor based on the selected file"""
    ...
    def get_files(self):
        ...
        if isdir(dir_path):
            self.search_sub_folders_last_status = (
                self.search_sub_folders.GetValue()
            )
            self.file_paths = sorted(
                get_file_paths(
                    dir_path,
                    search_sub_folders=self.search_sub_folders.GetValue(),
                )
            )
        else:
            self.file_paths = []
    ...
    def value_is_valid(self):
        ...
        # each call requires left and right square brackets, an int between
        # them, and a valid function call
        ...
        for call in call_str:
            if "[" not in call and "]" not in call:
                ...
                if not (
                    param.isdigit()
                    or (param.startswith("-") and param[1:].isdigit())
                ):
                    return False
    ...
    def a_referenced_tag_exists(history):
        return any(
            ["Referenced%s" % row[0] in list(keyword_dict) for row in history]
        )
    ...
    def set_output_paths(self, check_only=False):
        ...
        :param check_only: Do loop without save to check for overwriting
        ...
        for file_path, ds in self.ds.items():
            ...
            if self.retain_rel_dir.GetValue():
                rel_out_path = join(
                    output_dir, relpath(dirname(file_path), input_dir)
                )
                if not check_only:
    ...
    def call_next_value_generator(self):
        """Call each Value Generator one at a time, then call apply_edits"""
    ...
    # -------------------------------------------------------------------------
    # Step 3: View error log, check for file over-writing,
    # sync referenced tags, save DICOM to file
    # -------------------------------------------------------------------------
    def do_save_dicom(self):
        # If any errors, display in GUI and ask user if they'd like to continue
        if self.error_log:
    ...
    # -------------------------------------------------------------------------
    # Step 4: Save history, if requested, reparse original data since it has
    # been edited directly
    # -------------------------------------------------------------------------
    def on_save_complete(self):
        # If in and out directories are the same, need to update file list
        # and datasets with new files
    ...
    def save_history_to_file(self):
        save_file_name = "DVHA_DICOM_Editor_history_%s.csv" % str(
            datetime.now()
    ...
    def on_report_issue(evt):
        webbrowser.open_new_tab(
            "https://github.com/cutright/DVHA-DICOM-Editor/issues"
        )
...
class MainApp(wx.App):
    def OnInit(self):
        ...
        self.frame = MainFrame(
            None, wx.ID_ANY, "DVHA DICOM Editor v%s" % __version__
        )
